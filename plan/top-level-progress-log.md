...
> **这份文档是什么**：项目架构层全景 + 决策落实清单 + 已完成工作索骥 + 待完成工作总纲 + 未成熟想法暂存处。
...
> 注一新想法：前端右键菜单功能退化，是否考虑融入reference内（待开session讨论后落实）
...
| 序   | 项                                            | 来源         | 依赖                         | 开工指引                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---- | --------------------------------------------- | ------------ | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| 3 ✅ | 想法 2 的 (b) bind-by-id + enrichment sidecar | 【五】想法 2 | 无（可与 #4 合并，也可先做） | **端到端+frozen 验证完成**（2026-07-14）。代码+39 单测全绿；frozen exe 跑通（sidecar 落库正确、is_cited 标记对、幻觉检测触发）；dev 跑通（rich 引用完整、lean 后台可见）。验证中发现并修 2 个展示层 bug：doi 未规范化成 URL（`db25767`）、前端 ref 区 url 不可点击（`1751e3e`，更正「前端零改」前提）。随后做 url 语义专责化：候选层 `url` 只存真 PDF 直链（去掉落地页回退），展示层改双链接 `[DOI] · [PDF]`（分隔符用 `·` 避开前端 ` | `切段）。详见`plan/citation-bind-by-id-plan.md` |
