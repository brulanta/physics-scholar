...
| 序  | 项                                            | 来源         | 依赖                         | 开工指引                                                                                                                                                                     |
| --- | --------------------------------------------- | ------------ | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3 ✅ | 想法 2 的 (b) bind-by-id + enrichment sidecar | 【五】想法 2 | 无（可与 #4 合并，也可先做） | **端到端+frozen 验证完成**（2026-07-14）。代码+36 单测全绿；frozen exe 跑通（sidecar 落库正确、is_cited 标记对、幻觉检测触发）；dev 跑通（rich 引用完整、lean 后台可见）。验证中发现并修 2 个展示层 bug：doi 未规范化成 URL（`db25767`）、前端 ref 区 url 不可点击（`1751e3e`，更正「前端零改」前提）。详见 `plan/citation-bind-by-id-plan.md` |
