...
**过取口径（读数前先对齐，免重蹈归因错误）**：eval 跑分时 `topk=TOPK=10` → `fetch_k = RAG_FETCH_MULTIPLIER(10) × 10 = 100`。`hybrid_search` 两路**各取 100**（向量 `similarity_search(k=100)`、BM25 `[:100]`），RRF 融合去重后**再截回 100** 喂给 reranker（即候选池 M=fetch_k=100）。
⚠ 注意 eval 池子(100) ≠ **生产默认池子**：生产 `rag_tool` 的 `k` 默认 **5** → `fetch_k=50`。eval 用的池子比线上默认大一倍，故 eval 的绝对天花板偏乐观；这是除自指性外的第二个「eval 比线上乐观」的来源。

**结论（修订版——修正前一版的归因错误）**：
> ⚠ **前一版 plan 写错过**：曾称「B 的 R@10=0.80 > A+rerank 的 0.70 上限」。事实是 **A+rerank R@10 = 0.80**（那个 0.70 是 `probe_rerank.py` 探针 rerank@**50** 池的天花板，被错误串成 eval 的 A+rerank-**100** 池）。eval 池从 50 扩到 100，稠密天花板本就从 ~0.70 抬到 ~0.80。下面是修正后的归因。

1. **重排的贡献（最大、最稳）= C vs B，同一个 hybrid-100 池**：B 不重排（RRF 排序，R@1 仅 0.30）→ C 重排（R@1=1.0）。同池对比，排除候选池差异，**纯粹是 cross-encoder 排序质量**——这条最干净、受自指性影响最小（池子里有没有 gold 与重排无关，重排只管排序）。
2. **BM25 的贡献 = C vs A+rerank 在 R@10：0.80→1.0**（**不是** B vs A+rerank——这俩 R@10 都是 0.80，并列，BM25 增益不体现在这里）。稠密-100 池漏掉的 ~20%（约 4 题）gold，被 BM25 字面命中补进池，再由重排顶上来。
3. **生产默认 `RAG_HYBRID_ENABLED=True` + `RERANK_ENABLED=True` 保留**——重排是被最干净验证的最强单针；BM25 方向正确（见下免责声明对幅度的保留）。

**⚠ 免责声明（自指性偏差——不止 C，也部分影响 B 的增益幅度）**：
- **机制**：query 由 `eval_fixed` 某 chunk 生成，gold 答案句逐字摘自**同一 chunk**，命中判定看「检索片段覆盖 gold ≥0.6」。故 query 与源 chunk **共享大量精确术语字面**。
- **后果 1（C 绝对值虚高）**：源 chunk 对 gold 覆盖≈1.0，且字面高度重合 → BM25 必然捞回源 chunk → 重排必然顶到 rank1。C=1.0 含相当比例「检索到出题源 chunk」的半送分效应。
- **后果 2（更隐蔽：BM25 增益幅度也被放大，不只 C）**：这套测试集**系统性更利好字面匹配能力强的方案**（BM25/hybrid），并相对低估纯语义检索的真实能力。因为真实用户提问**很少逐字复用论文原句措辞**，而 LLM 合成出题倾向贴着原句改写（常见合成数据偏差）。所以 **C vs A+rerank 那个 R@10 0.80→1.0 的 BM25 补池增益，在真实 query 下大概率会缩水**——「BM25 有用」这个**方向**可信（学术域里 MPF/Q值/模型名/公式名等精确术语确会出现在真实 query），但「增益幅度」被合成偏差放大了多少，当前这批数据答不了。这一层比 C=1.0 更隐蔽，因为 B/C 的中间数字不像 1.0 那样自带警报。
- **可信 vs 不可信小结**：相对**方向**（重排 > 无重排、hybrid ≥ 纯稠密、新切片 > 旧切片）可信；**绝对值与增益幅度**偏乐观，受自指性 + eval 池(100)>生产池(50)双重抬高。叠加 n≈20，本评测「方向性参考，非统计严谨」。
- **更严格的独立校验（未来若需）**：需一批**不从 chunk 直接出题、贴近真实提问风格**的样本（换问法、不复用原句措辞、跨 chunk 综合提问），作为独立验证集，才能量化真实 query 下的增益。

**chunk-size 扫描——降级为「当前证据支持取消，保留重启口」（原『正式取消』表述过满）**：
- 取消的理由（hybrid+rerank 已大幅缓解大 chunk 稀释/hubness，不重切不重嵌）**方向成立**，但支撑它的关键数字（C=1.0、B/C 的增益幅度）正是本轮**最该被怀疑、偏乐观**的两层。故此决定的 confidence 没有「正式取消」听起来那么扎实。
- **决定**：当前维持不扫 chunk-size（不烧嵌入额度）。**但标注为：若未来真实用户反馈显示纯语义检索仍有明显短板（如术语不匹配时召回差），应重新评估扫描 256/200 档**，不完全关闭这条路。

**RAG 修复升级四部分（Part 1 切片 + Part 2 多路召回 + Part 3 重排 + Part 4 评测）全部落地。方向性结论已验证；绝对增益幅度待真实 query 校验集进一步确认（见上免责声明）。**
