...
- **阶段 3（收尾）✅ 已完成（2026-06-30 自动化 + 2026-07-01 实机核验）**：至此 MCP 迁移代码侧全部完成、6 工具全部 MCP 化、frozen 打包实证 + 实机 GUI/写后读/孤儿回收/热重载均通过。（两项遗留见「实机核验回报」末，不属 MCP 迁移范畴。）
  ...
  - **实机核验回报（2026-06-30 / 07-01）**：
    ...
    - **遗留（本次不入库，下次工作再定归属）**：① 选择重启后旧程序未能重新拉起、且旧前端窗口异常跟随消失（`on_restart` 的 `subprocess.Popen([sys.executable])` frozen 下重启链待查）；② 打包未重跑 `npm run build`，exe 内前端为 06-04 旧产物，外观与最新源码有差（定稿分发前须先 `cd frontend && npm run build` 再 `pyinstaller`）。
