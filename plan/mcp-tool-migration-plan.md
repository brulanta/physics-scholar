...
- **阶段 3（收尾）✅ 已完成（2026-06-30 自动化 + 2026-07-01 实机核验）**：至此 MCP 迁移代码侧全部完成、6 工具全部 MCP 化、frozen 打包实证 + 实机 GUI/写后读/孤儿回收/热重载均通过。（两项遗留见「实机核验回报」末，不属 MCP 迁移范畴。）
  ...
  - **实机核验回报（2026-06-30 / 07-01）**：
    ...
    - **遗留（2026-07-01 专项处理，代码侧已落地；遗留 1 待实机核验证实假设）**：
      ...
      **遗留 1 — frozen 托盘「重启」拉不起新进程 + 旧前端窗口异常跟随消失 —— ✅ 代码修复已落地（2026-07-01），待实机核验**
      ...
      - **⚠️ 2026-07-01 实机补记 + 漏修修正**：上一轮只补了托盘路径，**漏了第二条重启路径**——配置页「立即重启」按钮走 `POST /api/config/restart`（[routes.py:102](../src/api/routes.py#L102)），同样 `Popen → os._exit(0)` 但没加 breakaway。用户实机点「立即重启」→ 前后端全关、无自动拉起（**这条漏网路径失败，反而印证 Job 假设成立**）。补漏已在新 plan [startup-restart-refactor-plan.md](./startup-restart-refactor-plan.md) 的任务 A 落地（routes.py 补 breakaway）。**后续「启动/重启流程」相关工作（含并行拉起、强制重启去双跳）迁往该新 plan，本 MCP plan 就此封存为已闭环迁移记录。**
