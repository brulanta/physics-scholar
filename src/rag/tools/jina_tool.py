...
def jina_tool(
    ...
    ## 使用时机
    仅在 s2_search_tool / arxiv_tool 返回的摘要（abstract / tldr）已无法满足
    用户追问时调用本工具。典型场景：
      - 用户追问文中的具体实验设置、数据集细节、方法实现细节等
      - 摘要中未提及但用户明确需要的信息

    常规检索和摘要阅读请用 s2_search_tool，不要无故调用本工具。
    本工具会消耗额外 API（副 LLM）资源，请告知用户。

    ## 两种模式

    ### 无 query：全文截断模式
    返回全文开头 no_query_max_tokens 估算 token 的内容。
    不消耗副 LLM，适合快速了解文章结构或让用户决定是否深入查询。

    ### 有 query：分片打分模式
    对全文完整分片，逐片调副 LLM 打分（1-10），
    按分数从高到低累积返回，直到达到 max_return_tokens 预算或 top_n 上限。
    返回的是原始片段文本（非摘录），附带各片分数，供 agent 直接引用。
    ...
    - 副 LLM 未配置时，有 query 的请求会直接返回错误（不会静默降级）
    - 相同 URL 请求失败后 120 秒内不会重复发起 Jina 请求
